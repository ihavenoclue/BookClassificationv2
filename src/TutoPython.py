...
mon_dictionnaire = {'pseudo', 'mot de passe'} #this is NOT a dictionnary, but a SET. Like a list, but can't contain the same object twice
...
#The file is closed at the end EVEN IF AN EXCEPTION IS RAISED
...
class Compteur:
    ...
    #The following is CLASS METHOD (that can be used by calling directly Compteur.combien() instead of on an instanciated object)
    def combien(cls):
...
#Particular attributes where we ourselves define what happens when we try to read them or modify them
#Also possible to define what happens when we delete the attribute (del) or call for help on the attribute (less often used)

#NOTE : THERE ARE NO PUBLIC OR PRIVATE ATTRIBUTES IN PYTHON. Instead, we use properties to "hide" the parts you want the user not to use.

#The whole point is to make it appear seamless to the user. He's accessing an attribute as usual (so he belie
#but we want special stuff to happen when he does so we define it as a property.

#4 methods for properties: accesseur , mutateur, suppression, aide. Below we use accesseur and mutateur
...
class Personne:
    ...
    """Méthode qui sera appelée quand on souhaitera accéder en lecture
        à l'attribut 'lieu_residence' , par exemple par andre.lieu_residence"""
    ...
    # propriété
    lieu_residence = property(_get_lieu_residence, _set_lieu_residence) # note that no underscore is in front. Here we say to Python that lieu_residence is a property with with set and get methods
...
#If we want class B to herit from A, i.e. B has all the attributes and methods from A:
class B(A):
...
# If we do object_from_B.method() , Python will first look if the method is defined in B. If not, it will iteratively
# climb the hierarchy up to search for the method in the "mother" class

# THIS IS  ALSO TRUE FOR THE CONSTRUCTOR ! i.e. if you define a constructor for B _init_ that doesn't call the one from A, 
# you won't get the attributes defined in A.
...
#python automatically chooses the sorting method based on the type of objects within the list.
#if all numbers, than number orders. If strings,then alphabetical. If mix, then error is returned.
